...
export class HomePageComponent implements OnInit {
  ...
  constructor(private locationStrategy: LocationStrategy, private restApi: RestAPIService,
    private authService: AuthService, private router: Router, private _datePipe: DatePipe) { }
  ...
  ngOnInit() {
    ...
    this.OverAllSLAtext = this.assignYearQuaterly();
  ...
  assignYearQuaterly(): string {
    var result = '';
    var curr_month = new Date().getMonth() + 1;
    var curr_year = new Date().getFullYear();
    var month = new Date().toLocaleString('default', { month: 'short' });
    // if(curr_month <= 3) {
    //   result = 'Overall SLA Status For Q1(Jan to '+ month + ' ' + curr_year + ')';
    // } else if(curr_month > 3 && curr_month <= 6) {
    //   result = 'Overall SLA Status For Q2(Apr to '+ month + ' ' + curr_year + ')';
    // } else if(curr_month > 6 && curr_month <= 9) {
    //   result = 'Overall SLA Status For Q1(Jul to '+ month + ' ' + curr_year + ')';
    // } else if(curr_month > 9 && curr_month <= 12) {
    //   result = 'Overall SLA Status For Q1(Oct to '+ month + ' ' + curr_year + ')';
    // } else {
    //   result = '';
    // }
    // return result;
    return result = 'Overall SLA Status';
  }
